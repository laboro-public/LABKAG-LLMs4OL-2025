...
def main():
    ...
    parser.add_argument("prompt", choices=["1", "2"], help="choose between EE Prompt 1 & 2")
    ...
    for subset in subsets:
        ...
        if args.prompt == "1":
            ...
            with open(os.path.join(args.data_dir, f'{subset}/train/documents.jsonl'), 'r') as f:
                train_example_docs = [json.loads(line) for line in f if json.loads(line)['id'] in example_outputs]
        ...
        for doc in tqdm(test_docs, desc=f"Processing {subset}"):
            ...
            contents[doc['id']] = qwen_gen(prompt_template, 
                                        tokenizer,
                                        model,
                                        **prompt_template_kwargs)
