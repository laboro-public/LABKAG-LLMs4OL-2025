...
def raw_ee_line2entity(entity_line, lower=True):
    if lower:
...
def raw_ee_line2cat(entity_line, lower=True):
    if lower:
...
def raw_ee_line2des(entity_line, lower=True):
    if lower:
...
def ee_prompt_2_preprocess(
    ...
    for doc in train_example_docs:
        example_strings.append(example_string_template.format(
            title = doc['title'],
            text = doc['text'],
...
def ec_prompt_1_preprocess(ee_contents):
...
def ec_prompt_1_generate(
    ...
    for line in tqdm(list_of_entities_with_description.splitlines(), desc=f'Processing {subset}'):
        class_response = qwen_gen(**qwen_gen_kwargs,
                                  list_of_entities_with_description=line)
        # Process raw response into JSON format
        try:
            all_class_responses += json.loads(class_response)
        except:
            try:
                all_class_responses.append(json.loads(re.findall(r"\[\s*{.*?}\s*\]", class_response, re.DOTALL)[0])[0])
            except:
...
def ec_prompt_1_postprocess(all_class_responses):
    extracted_terms = []
    extracted_types = []
...
def ec_prompt_2_preprocess(
    ...
    for train_doc in example_train_docs:
        doc_id = train_doc['id']
        entity_list = example_doc_entities[doc_id]['terms']+example_doc_entities[doc_id]['types']
        random.shuffle(entity_list)
        example_output_list = []
        for entity in entity_list:
            if entity in example_doc_entities[doc_id]['terms']:
                example_output_list.append(
                    f'{entity}|||term'
                )
            else:
                example_output_list.append(
        ...
        example_list.append(example_template.format(
...
def ec_prompt_2_postprocess(contents):
    extracted_terms = []
    extracted_types = []
