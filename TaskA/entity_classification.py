...
def main():
    ...
    parser.add_argument("prompt", choices=["1", "2"], help="choose between EC Prompt 1 & 2")
    parser.add_argument("--data_dir", "-d",
                        default="data/LLMs4OL-Challenge/2025/TaskA-Text2Onto", 
                        help="path to input data directory")
    parser.add_argument("--example_dir", "-e",
                        default="data/few_shot_examples", 
                        help="path to few-shot examples directory")
    parser.add_argument("--ee_output_dir", "-p",
    ...
    for subset in subsets:
        ...
        with open(os.path.join(testdata_dir, f'text2onto_{subset}_test_documents.jsonl'), 'r') as f:
        ...
        if args.prompt == "1":
            ...
            extracted_terms, extracted_types = ec_prompt_1_postprocess(all_class_responses)

        elif args.prompt == "2":
            example_template = plain_classification_customized_example_string
            if subset == 'engineering':
                prompt_template = plain_classification_engineering_customized_examples_prompt
            elif subset == 'scholarly':
                prompt_template = plain_classification_scholarly_customized_examples_prompt

            example_doc_entities = json.load(open(os.path.join(
                args.example_dir, f'entity_classification/{subset}/example_doc_entities.json')))

            with open(os.path.join(args.data_dir, f'{subset}/train/documents.jsonl')) as f:
                example_train_docs = [json.loads(line) for line in f if json.loads(line)['id'] in example_doc_entities]

            with open(os.path.join(args.data_dir, f'{subset}/test/text2onto_{subset}_test_documents.jsonl')) as f:
                test_docs = [json.loads(line) for line in f]
            
            examples_string, extracted_entities = ec_prompt_2_preprocess(
                example_train_docs,
                example_doc_entities,
                example_template,
                ee_contents)
            ...
            output_dir = os.path.join(args.output_dir, subset)
            os.makedirs(output_dir, exist_ok=True)
            json.dump(contents, open(os.path.join(output_dir, 'contents.json'), 'w'), indent=2)
